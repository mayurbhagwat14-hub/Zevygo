...
const AdminSettings = () => {
    ...
    const loadFinancialSettings = async () => {
      try {
        ...
        if (res.success && res.settings) {
          ...
          setBrandingSettings({
            appName: res.settings.appName || 'App',
  ...
  const handleBrandingSave = async (e) => {
    ...
    try {
      await updateSettings(brandingSettings);
      toast.success('Branding settings updated');
      // Dispatch event to update globally if needed without reload
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      {isSuperAdmin && (
        <div onClick={() => setActiveView('branding')}
          ...
          <h3 className="text-lg font-bold text-gray-800 mb-2">Branding Settings</h3>
          <p className="text-sm text-gray-500">Manage application name and logo</p>
        </div>
      )}
    </div>
  );
        ...
        {/* Branding View */}
        {
          activeView === 'branding' && (
            <motion.div key="branding" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
              <div className="max-w-2xl mx-auto bg-white rounded-xl p-8 shadow-sm border border-gray-100 h-fit">
                <div className="flex items-center gap-3 mb-6">
                  <div className="p-2 bg-pink-100 rounded-lg">
                    <FiImage className="w-5 h-5 text-pink-600" />
                  </div>
                  <div>
                    <h2 className="text-lg font-bold text-gray-800">Application Branding</h2>
                    <p className="text-xs text-gray-500">Update global app name and logo</p>
                  </div>
                </div>
                ...
                <form onSubmit={handleBrandingSave} className="space-y-6">
                  <div>
                    ...
                    <input type="text" name="appName" value={brandingSettings.appName} onChange={handleBrandingChange} required
                      placeholder="e.g., Homester"
                      className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none transition-all" />
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Application Logo</label>
                    <div className="space-y-3">
                      <input type="url" name="appLogo" value={brandingSettings.appLogo} onChange={handleBrandingChange}
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            ...
                            if (file) {
                              ...
                              try {
                                const url = await uploadToCloudinary(file, 'Homster/branding');
                                setBrandingSettings(p => ({ ...p, appLogo: url }));
                          ...
                          className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 transition-all bg-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-pink-50 file:text-pink-700 hover:file:bg-pink-100 disabled:opacity-50 disabled:cursor-not-allowed"
                        />
                      </div>
                      ...
                      <p className="text-xs text-gray-500">Provide an image URL or upload a file directly. Leave empty to use the default logo.</p>
                    </div>
                  </div>
                  ...
                  {brandingSettings.appLogo && (
                    <div className="mt-4 p-4 border rounded-lg bg-gray-50">
                      ...
                      <img key={brandingSettings.appLogo} src={brandingSettings.appLogo} alt="Logo Preview" className="h-12 object-contain" onError={(e) => e.target.style.display = 'none'} />
                    </div>
                  )}

                  <div className="flex justify-end pt-4">
                    <button type="submit" disabled={brandingLoading}
                      className="px-8 py-3 bg-pink-600 text-white rounded-xl font-medium hover:bg-pink-700 flex items-center gap-2 disabled:opacity-70 shadow-lg shadow-pink-200 transition-all">
                      {brandingLoading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <FiSave className="w-5 h-5" />}
                      Update Branding
                    </button>
                  </div>
                </form>
