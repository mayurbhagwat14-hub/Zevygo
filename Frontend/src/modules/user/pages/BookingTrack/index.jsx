    ...
    <div className="h-screen flex flex-col relative bg-white overflow-hidden">
      ...
      <div className="flex-1 w-full h-full">
        ...
        <button
          ...
          className={`absolute top-40 right-4 p-4 rounded-full shadow-2xl transition-all active:scale-90 z-20 ${isAutoCenter ? 'bg-primary-600 text-white animate-pulse' : 'bg-white text-gray-700'}`}
          style={{ boxShadow: '0 8px 30px rgba(0,0,0,0.2)' }}
        >
      ...
      <div className={`absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-[0_-8px_30px_rgba(0,0,0,0.12)] z-20 p-6 pb-8 transition-transform duration-300 ${isFullScreen ? 'translate-y-full' : ''}`}>
        ...
        <div className="bg-gray-50 rounded-2xl p-4 flex items-start gap-4 mb-4 border border-gray-100">
          <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-md text-primary-600 border border-gray-100 shrink-0">
        ...
        {booking?.status?.toLowerCase() === 'visited' && !(booking.arrivalOTP || booking.visitOtp) && (
          <div className="mb-4 relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary-500 to-emerald-700 p-4 shadow-lg flex items-center gap-4">
        ...
        {!booking?.customerConfirmationOTP && booking?.status?.toLowerCase() === 'work_done' && !booking?.cashCollected && (
          <div className="bg-white rounded-2xl p-4 shadow-lg border border-primary-100 mb-4 flex items-center gap-4 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-20 h-20 bg-primary-50 rounded-full -translate-y-10 translate-x-10 blur-2xl"></div>
