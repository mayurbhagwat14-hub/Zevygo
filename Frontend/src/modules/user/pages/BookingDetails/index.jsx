...
const BookingDetails = () => {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (response.data?.success && response.data?.settings) {
          ...
          setSupportInfo({
            email: supportEmail || `help@${branding.appName.toLowerCase().replace(/\s+/g, '')}.com`,
        ...
        setSupportInfo({
          email: `help@${branding.appName.toLowerCase().replace(/\s+/g, '')}.com`,
          phone: '+919999999999'
        });
      }
    };
  ...
  if (!booking) {
    return (
      <div className="min-h-screen flex items-center justify-center relative bg-neutral-50">
        ...
        <div className="relative z-10 px-6 w-full max-w-md">
          <EmptyState
            ...
            message="This booking may have been removed or the link is invalid."
            actionLabel="Go to My Bookings"
            onAction={() => navigate('/user/my-bookings')}
          />
        </div>
      </div>
    );
      ...
      <div className="relative z-10">
        <header className="sticky top-0 z-40 backdrop-blur-xl bg-white/80 border-b border-neutral-100 px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <Button type="button" variant="icon" onClick={() => navigate('/user/my-bookings')} aria-label="Back to bookings">
              <FiArrowLeft className="w-5 h-5" />
            </Button>
            <div className="flex-1 min-w-0">
              <h1 className="text-xl font-extrabold text-neutral-900 tracking-tight truncate">Booking Details</h1>
        ...
        <main className="max-w-xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button
              onClick={() => {
                const email = supportInfo.email || `help@${branding.appName.toLowerCase().replace(/\s+/g, '')}.com`;
                const link = document.createElement('a');
