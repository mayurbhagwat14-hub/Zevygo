...
const Settings = require('../models/Settings'); // Add Settings model
...
const getAppName = async () => {
  try {
    ...
    return settings && settings.appName ? settings.appName : 'Homster';
  } catch (error) {
    return 'Homster';
  }
};

const emailWrapper = (content, title, appName, preheader = '') => `
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <div class="wrapper">
    <div class="main">
      <div class="header">
        <div class="logo-circle">${appName.charAt(0)}</div>
        <h1>${appName}</h1>
      </div>
      <div class="content">
        ${content}
      ...
      <div class="footer">
        ...
        <p>&copy; ${new Date().getFullYear()} ${appName} India. All rights reserved.</p>
  ...
  try {
    ...
    const content = `
      <div style="text-align: center;">
        ...
        <p>Your one-time password (OTP) for ${appName} is ready. Use this code to complete your ${purpose.replace('_', ' ')}.</p>
    ...
    await transporter.sendMail({
      from: process.env.EMAIL_FROM || `${appName} <noreply@homster.com>`,
      to: email,
      subject: `${subjectPrefix} - ${appName}`,
      html: emailWrapper(content, subjectPrefix, appName, `Your verification code is ${otp}`)
    });
    return { success: true };
  } catch (error) {
